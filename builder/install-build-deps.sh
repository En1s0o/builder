...
echo
query_cmd="apt-get --just-print install -y $(echo $packages) --fix-missing"
if cmd_output="$(LANGUAGE=en LANG=C $query_cmd)"; then
  ...
  if [ -z "$new_list" ] && [ -z "$upgrade_list" ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y ${do_quietly-} ${new_list} ${upgrade_list} --fix-missing
  fi
  echo
else
