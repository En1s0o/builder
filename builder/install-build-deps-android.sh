...
# Fix deps
sudo apt-get -f -y install --fix-missing

# common
sudo apt-get -y install lib32z1 lighttpd python-pexpect xvfb x11-utils --fix-missing

# Some binaries in the Android SDK require 32-bit libraries on the host.
# See https://developer.android.com/sdk/installing/index.html?pkg=tools
sudo apt-get -y install libncurses5:i386 libstdc++6:i386 zlib1g:i386 --fix-missing

# Required for apk-patch-size-estimator
sudo apt-get -y install bsdiff --fix-missing
...
function CheckOrInstallJava() {
  if ! (IsJavaInstalled java $2 && IsJavaInstalled javac $2); then
    sudo apt-get -y install openjdk-$1-jre openjdk-$1-jdk --fix-missing
  fi
